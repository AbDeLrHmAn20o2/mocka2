  ...
  try {
    ...
    try {
      ...
      if (payload.exp && now - payload.exp > 2 * 60 * 60) {
        console.log(
          `🚨 Rejecting extremely stale token: ${now} vs ${payload.exp} (${
            now - payload.exp
          } seconds old)`
        );
        return res.status(401).json({
          ...
          details:
            "Token is more than 2 hours past expiry. Fresh authentication required.",
        });
      }
    } catch (decodeError) {
      console.log(
        "Failed to decode token for staleness check:",
        decodeError.message
      );
    ...
    if (errorMessage.includes("extremely stale")) {
      return res.status(401).json({
        error: "Token is extremely stale! Please login again.",
        code: "TOKEN_EXTREMELY_STALE",
        details: "Token is far beyond expiry. Fresh authentication required.",
      });
    }
